...
exports.create = function(connection, config, ready) {
    ...
    // couchfake: don't go RESTFUL for simplicity
    function view(ddoc, name, query, callback) {
        ...
        */
        connection.view(ddoc, name, query, viewHandler, [callback, this]);
    }
    ...
    function setDesignDoc(name, data, callback) {
        ...
        connection.setDesignDoc(name, makeDoc(data),
                                couchnodeRestHandler, [callback, this]);
        */
        connection.setDesignDoc(name, data, setDesignDocHandler, [callback, this]);
    }
    ...
    function getDesignDoc(name, callback) {
        ...
        */
        connection.getDesignDoc(name, getDesignDocHandler, [callback, this]);
    }
    ...
    function deleteDesignDoc(name, callback) {
        ...
        */
        connection.deleteDesignDoc(name, deleteDesignDocHandler, [callback, this]);
    }
...
function viewHandler(data, errorCode, results) {
    var error = makeError(data[1], errorCode);
    data[0](error, results);
}

function getDesignDocHandler(data, errorCode, value) { // TODO: parse doc?
    var error = makeError(data[1], errorCode);
    data[0](error, value);
}

function setDesignDocHandler(data, errorCode) { // TODO: makeDoc?
    var error = makeError(data[1], errorCode);
    data[0](error, {}); // TODO: what's return data?
}

function deleteDesignDocHandler(data, errorCode) {
    var error = makeError(data[1], errorCode);
    data[0](error, {}); // TODO: what's return data?
}
